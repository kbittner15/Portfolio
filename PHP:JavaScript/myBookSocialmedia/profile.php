...
if (isset($_GET['username'])) {
        if (DB::query('SELECT username FROM users WHERE username=:username', array(':username'=>$_GET['username']))) {
                ...
                $tokenid = DB::query('SELECT user_id FROM tokens WHERE token=:token', array(':user_id'=>$_GET['user_id']));
                $username = DB::query('SELECT username FROM users WHERE username=:username', array(':username'=>$_GET['username']))[0]['username'];
                $username2 = DB::query('SELECT username FROM users WHERE id=:tokenid', array(':username'=>$_GET['username']))[0]['username'];
               ...
               if (isset($_POST['follow'])) {
                ...
                if (isset($_POST['post'])) {
                    ...
                    Post::createPost($_POST['postbody'], Login::isLoggedIn(), $userid, $_POST['public']);
...
<form action="profile2.php?username=
<?php echo $username; ?>
" method="post">
        <?php
        if ($userid != $followerid) {
...
<form action="profile2.php?username=<?php echo $username; ?>" method="post" enctype="multipart/form-data">
